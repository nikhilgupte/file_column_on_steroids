...
module FileColumn # :nodoc:
  ...
  class TempUploadedFile < RealUploadedFile # :nodoc:
    ...
    def parse_temp_path(temp_path, instance_options=nil)
      ...
      ##@tmp_dir, @filename = $1, FileColumn.sanitize_filename($3) # commented it out cos the dynamin filenaming intereferes with it.
      @tmp_dir, @filename = $1, $3
      @dir = File.join(tmp_base_dir, @tmp_dir)
  ...
  class PermanentUploadedFile < RealUploadedFile # :nodoc:
    ...
    def relative_path_prefix
      ...
      #@instance.id.to_s
      FileColumn::split_into_multi_dirs(@instance.id)
    end
  end
  ...
  module ClassMethods
    ...
    def file_column(attr, options={})
      ...
      if options[:s3]
        include FileColumn::S3FileColumnExtension
        self.s3_file_column(attr)
      end
    end
  ...
  def self.sanitize_filename(filename)
    ...
    filename = "#{Time.now.to_i}#{filename.slice(filename.rindex('.'), filename.length)}"
    filename
  end

  def self.sanitize_filename_old(filename)
    filename = File.basename(filename.gsub("\\", "/")) # work-around for IE
  ...
  def self.split_into_multi_dirs(id)
    dir = id
    path = ''
    while ((q = dir/100) > 0)
      path = '/' + (dir % 100).to_s.rjust(2, '0') + path
      dir = q
    end
    path = (dir % 100).to_s.rjust(2, '0') + path + "/N#{id}"
    path
  end
end
