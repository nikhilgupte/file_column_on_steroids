 ...
 * In the original, all files were stored into one single folder per column. This would result in maxing out the limit
   of the number of sub-directories within a directory. This version addresses this problem by breaking up the
	 sub-directories such that there will never be more than 101 sub-directories directly with a directory.
